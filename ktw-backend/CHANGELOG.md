...
### ✨ 新功能：房間確認狀態 API + 每日自動清除

#### 1. 資料庫層 (SQLite)

**檔案**: `src/helpers/db.js` (L455-585)

- **新資料表**: `room_acknowledgments`
  ```sql
  CREATE TABLE room_acknowledgments (
  ...
  ```
- **新增函數**:
  - `getRoomAcknowledgments(date)` - 取得當日已確認房間列表
  - `addRoomAcknowledgment(roomNumber, date)` - 新增確認記錄
  - `clearAllAcknowledgments()` - 清除所有確認記錄

#### 2. API 端點

**檔案**: `src/index.js` (L256-290)
| 方法 | 端點 | 說明 |
|-----|------|------|
| GET | `/api/room-acknowledgments` | 取得當日已確認房間 |
| POST | `/api/room-acknowledgments` | 新增房間確認記錄 |

#### 3. 每日 23:00 自動清除

**檔案**: `src/helpers/db.js` (L555-580)

- **定時任務**: 使用 `setTimeout` + `setInterval` 每天 23:00 清除所有確認記錄
- **日誌**: 清除後輸出 `🕐 每日 23:00 定時清除：已清除 X 筆房間確認記錄`
...
1. **查詢臨時訂單** (L549-567)
   ...
   const sameDayRes = await fetch(
     "http://192.168.8.3:3000/api/bookings/same-day-list"
   );
   const targetBooking = bookings.find(
     (b) => b.item_id === order_id || b.order_id === order_id
   );
   ```
...
2. **查詢 PMS 今日入住名單** (L569-578)
   ...
   const pmsRes = await fetch(
     "http://192.168.8.3:3000/api/bookings/today-checkin"
   );
   ```
