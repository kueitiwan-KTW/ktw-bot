...
**檔案**: `src/App.vue` (L755-810), `src/style.css` (L560-576)

- **功能**: 今日有排房的房間在待處理面板中**紅色閃爍**提示
- **邏輯**: 不管清潔狀態，只要是今日入住房號就顯示並閃爍
- **點擊確認**: 點擊閃爍房間後停止閃爍，當天只能點擊一次
- **排除維修房**: `room_status.code === 'R'` 的維修房不顯示

#### 2. 跨電腦同步確認狀態

**檔案**: `src/App.vue` (L700-735)

- **功能**: 確認狀態存入後端 SQLite 資料庫，多台電腦同步
- **API**:
  - `GET /api/room-acknowledgments` - 載入當日已確認房間
  - `POST /api/room-acknowledgments` - 儲存確認記錄
- **移除**: 不再使用 `localStorage` 暫存

#### 3. 待處理房間樓層分組排版

**檔案**: `src/App.vue` (L801-810), `src/style.css` (L384-413)

- **功能**: 按樓層（2F, 3F, 5F, 6F）分組顯示待處理房間
- **排版**: 每層樓一橫列，樓層標籤在左側

#### 4. 每日 23:00 自動清除

**檔案**: `ktw-backend/src/helpers/db.js` (L555-580)

- **功能**: 每天晚上 23:00 自動清除當日房間確認記錄
- **實作**: 使用 `setTimeout` + `setInterval` 定時任務

### 📝 修改的文件

- `src/App.vue` - 閃爍邏輯、API 調用、樓層分組
- `src/style.css` - 樓層排版樣式、閃爍動畫（紅色）
...
1. **mismatch 訂單取消按鈕失效**
   - **檔案**: `src/App.vue` (L312)
   - **問題**: `cancelAllBookings` 過濾條件未包含 `mismatch` 狀態
   - **修復**: 加入 `|| i.status === 'mismatch'`

### ✨ 新功能

1. **當日預訂顯示 LINE 姓名**
   - **檔案**: `src/App.vue` (L845)
   - **修改**: 顯示 `guest_name` 和 `line_display_name`
   - **格式**: `👤 王小明 (LINE姓名) | 📞 ... | 🕐 ...`
...
1. **真實 PMS API 數據整合**
   ...
   - **實作**:
     ```javascript
     const pmsStats = await fetch("/api/pms/dashboard").then((res) =>
       res.json()
     );
